    ...
    for file in daterange2files(fromdate, todate):
        ...
        try:
            for line in gzip.open(file, "rb"):
                try:
                    line1 = line.decode('utf-8').encode('utf-8')
                except:
                    line1 = line.decode('utf-8','replace').encode('utf-8','replace')
                line1 = line1.lstrip()
                while line1:
                    (outp, ix) = decoder.raw_decode(line1)
                    yield outp
                    line1 = line1[ix:].lstrip()
        except IOError, ioe:
            sys.stderr.write(str(ioe) + "\n")
        except ValueError, ve:
            sys.stderr.write("Skipping line in "+ file+ " because"+ str(ve) + "\n")
        except UnicodeDecodeError, ude:
            sys.stderr.write("Skipping line in "+ file+ " because"+ str(ude) + "\n")
